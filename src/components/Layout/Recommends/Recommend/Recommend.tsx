...
export default function Recommend({ session }: { session: any }) {
  const {
    data: recommends,
    isLoading,
    isError,
  ...
  return (
    <>
      <button className={`${BUTTONSTYLE} slide-prev`}>
        <Image
          className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] md:w-auto"
              ...
              >
                <div className="bg-[#DBEBFF] border border-SubColor4 py-5 px-4 rounded-lg cursor-pointer group-hover:bg-primary transition-all">
                  <div className="flex flex-wrap gap-[5px] leading-none">
                    <div className="py-1 px-[6px] whitespace-nowrap rounded text-[13px] text-[#4378FF] bg-[#BFE0FF]">
                      {STATUS[e.meetingStatus]}
                    </div>
                    <div className="py-1 px-[6px] whitespace-nowrap rounded text-[13px] text-[#54A900] bg-[#D2FFAE]">
                      {GAMETYPE[e.gameType]}
                    </div>
                    <div className="py-1 px-[6px] whitespace-nowrap rounded text-[13px] text-[#FF6414] bg-[#FDC9AF]">
                      {LOCATION[e.location]}
                    </div>
                  </div>
                  <h4 className="font-medium text-base leading-[1.3] mt-[11px] break-keep text-ellipsis line-clamp-2 h-10 group-hover:text-white">
                    {e.title}
                  </h4>
                  <ul className="hidden md:block mt-8 text-[13px]">
                    {e.meetingDateTime && (
                      <li className="flex text-primary group-hover:text-white">
                        ...
                        <p className="border-l border-primary group-hover:border-white pl-[9px]">
      ...
      <button className={`${BUTTONSTYLE} slide-next`}>
        <Image
          className="absolute -scale-x-100 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] md:w-auto"
          src={"/asset/image/icon/slide-arrow.png"}
          width={22}
          height={20}
          alt="오른쪽 버튼"
